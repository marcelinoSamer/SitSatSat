...
def run_test_cases(test_cases_files):
    ...
    for i, test_case_file in enumerate(test_cases_files):
        ...
        result = {
            ...
            "runtime": runtime  # Adding runtime for the test case
        }
...
def print_results(results):
    ...
    ilp_runtimes = [result['runtime'] for result in results]
    ilp_satisfiability = [result['is_satisfiable'] for result in results]
    ...
    plt.plot(range(1, total_cases + 1), ilp_runtimes, marker='o', linestyle='-', color='blue')
    plt.xlabel('Test Case Index')
    plt.ylabel('Runtime (seconds)')
    plt.title('ILP Runtime per Test Case')
    plt.show()

    # Plotting satisfiability rate
    plt.figure(figsize=(10, 5))
    ilp_satisfiability_rate = (sum(ilp_satisfiability) / total_cases) * 100
    plt.bar(['ILP'], [ilp_satisfiability_rate], color='green')
    plt.xlabel('Approach')
    plt.ylabel('Satisfiability Rate (%)')
    plt.title('ILP Satisfiability Rate')
    plt.show()
