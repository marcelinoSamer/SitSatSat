...
for index, path in enumerate(sorted_files):
    ...
    print("How many times would you like the simulated annealer to run?")
    reads = int(input())    
    ...
    with open(result_path, "a") as file:
        ...
        for sample in solutions:
            file.write(str(sample).replace("np.int8(", "").replace(")", ""))
            file.write(
    ...
    solution_index= analyze_file.max_solvable(result_path)
    print("Do you want to see the variable assignment?    (y/n)" )
    s=input()
    if s=="y":
        print({k: int(v) for k, v in solutions[solution_index].items()})
